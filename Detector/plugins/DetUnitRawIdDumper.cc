...
class DetUnitRawIdDumper : public edm::one::EDAnalyzer<edm::one::WatchRuns> {
public:
  explicit DetUnitRawIdDumper(const edm::ParameterSet&);
  ~DetUnitRawIdDumper() = default;
...
DetUnitRawIdDumper::DetUnitRawIdDumper(const edm::ParameterSet& pset):
...
void DetUnitRawIdDumper::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void DetUnitRawIdDumper::beginRun(const edm::Run& run, const edm::EventSetup& eventsetup) {
...
DEFINE_FWK_MODULE(DetUnitRawIdDumper);